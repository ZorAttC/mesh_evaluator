...
#This file is derived from https://github.com/jianhengLiu/evaluate_3d_reconstruction_lib/blob/main/evaluate_3d_reconstruction/evaluate_3d_reconstruction.py
...
def eval_mesh2mesh(file_pred, file_trgt, 
                   down_sample_res=-1, 
                   threshold=0.05, 
                   truncation_acc=0.50, 
                   truncation_com=0.50, 
    ...
    mesh=trimesh.load(file_pred)
    if icp_align:
        transformation = get_align_transformation(file_pred, file_trgt)
    ...
    if pred_translate_to_zero:
        mesh_pred.vertices = o3d.utility.Vector3dVector(np.array(mesh_pred.vertices) - np.array(mesh_pred.vertices ).min(axis=0))
    if gt_translate_to_zero:
        mesh_trgt.vertices = o3d.utility.Vector3dVector(np.array(mesh_trgt.vertices) - np.array(mesh_trgt.vertices ).min(axis=0))
   # (optional) filter the prediction outside the gt bounding box (since gt sometimes is not complete enough)
    ...
    pcd_sample_gt=None
    if np.array(mesh_trgt.vertices).shape[0] < np.array(mesh_pred.vertices).shape[0]:
        pcd_sample_gt = mesh_trgt.sample_points_uniformly(
            number_of_points=np.array(mesh_pred.vertices).shape[0]
        )
        pcd_sample_pred = o3d.geometry.PointCloud(points=o3d.utility.Vector3dVector(mesh_pred.vertices))
    else:
        pcd_sample_gt = o3d.geometry.PointCloud(
            points=o3d.utility.Vector3dVector(mesh_trgt.vertices)
        )
        pcd_sample_pred = mesh_pred.sample_points_uniformly(
            number_of_points=np.array(mesh_trgt.vertices).shape[0]
        )
    
    if down_sample_res > 0:
        print("downsample enabled")
        pred_pt_count_before = len(pcd_sample_pred.points)
        gt_pt_count_before=len(pcd_sample_gt.points)
        print(pred_pt_count_before,gt_pt_count_before)
        pcd_sample_pred = pcd_sample_pred.voxel_down_sample(down_sample_res)
        print("pred")
        pcd_sample_gt = pcd_sample_gt.voxel_down_sample(down_sample_res)
        print("gt")
        pred_pt_count_after = len(pcd_sample_pred.points)
        gt_pt_count_after=len(pcd_sample_gt.points)
        print("Predicted mesh unifrom sample: ", pred_pt_count_before, " --> ", pred_pt_count_after, " (", down_sample_res, "m)")
        print("gt mesh unifrom sample: ", gt_pt_count_before, " --> ", gt_pt_count_after, " (", down_sample_res, "m)")
    
    verts_pred = np.asarray(pcd_sample_pred.points)
    verts_trgt = np.asarray(pcd_sample_gt.points)
